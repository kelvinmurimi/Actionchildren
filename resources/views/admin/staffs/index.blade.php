    ...
    <div>
        ...
        <div class="mt-5 mb-5 grid sm:grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="col-span-2">
                <x-form.input type="search" name="name" wire:model="name" label="none" :placeholder="__('Search staffs')" />
            </div>
        ...
        <div class="flex space-x-2 justify-end p-3">
            <a href="{{ route('admin.staffs.create') }}"class="btn btn-primary p-3">{{ __('Add staff') }}</a>
        ...
        <div class="overflow-x-scroll shadow-md">
            <table>
                <thead>
                    <tr>
                        <th><a href="#" wire:click.prevent="sortBy('first_name')">{{ __('Staff Name') }}</a></th>
                        ...
                        <tr>
                            <td class="flex">
                                <div>
                                    ...
                                    <img src="{{ asset($staff->image) }}" alt="{{ $staff->name }}" width="80"
                                        class="h-12 w-12 rounded-full">

                                </div>
                                <div class="pl-1 pt-1">{{ Str::limit($staff->bio, 100) }}</div>
                            </td>
                            <td>
                                <div class="flex space-x-2">
                                    <a
                                        href="{{ route('admin.staffs.edit', $staff->id) }}"class="btn btn-primary">{{ __('Edit') }}</a>
